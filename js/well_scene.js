...
WellColumn = function(well) {
  ...
  var top = well.well.top[0]*z_scale
  var bottom = well.well.bottom[0]*z_scale
  ...
  var mesh = WellMesh(geometry, material, well);
  mesh.position.setZ(-(top + (height / 2)))
  return mesh
...
Reservoir = function(well) {
  ...
  var top = well.reservoir.top * z_scale
  var bottom = well.reservoir.bottom * z_scale
  var height = (bottom - top)
